...
import { BAR_ADDRESS, Token, TokenAmount } from 'quest-regoswap-sdk'
...
export default function StakeCard({ sushiBalance, xSushiBalance }: StakeCardProps) {
    ...
    const [approvalState, approve] = useApproveCallback(     
        (chainId === 137)
        ? new TokenAmount(
            new Token(137, '0xbea6aff7067d1db51145039c6484a9b670c705ad', 18, 'REGO', ''),
            parsedInput.value.toString()
        )  
        :
        (chainId === 4)
        ? new TokenAmount(
            new Token(4, '0x26053DBBB42292C715C8405511d3EB9e5da032A7', 18, 'REGO', ''),
            parsedInput.value.toString()
        )
        :
        (chainId === 56)
                ...
                <div className="relative h-0 bottom-14 w-full pointer-events-none">
                    ...
                    >
                        <div className="flex">
                            ...
                            >
                                {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}REGO`}
                            </p>
                        </div>
